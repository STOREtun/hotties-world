        ...
        private IBillingService instantiateBillingSubsystem() {
            switch (config.CurrentPlatform) {
                case BillingPlatform.AppleAppStore:
                    return new AppleAppStoreBillingService(config, getMapper(), platformProvider.getStorekit(), util, getLogger());
                ...
                case BillingPlatform.WindowsPhone8:
                    ...
                    var result = new WP8BillingService(unibill.Dummy.Factory.Create(config.WP8SandboxEnabled, GetDummyProducts()), config, getMapper(), getTransactionDatabase(), util, getLogger());
