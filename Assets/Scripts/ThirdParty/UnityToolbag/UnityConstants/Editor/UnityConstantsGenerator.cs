    ...
    public static class UnityConstantsGenerator
    {
        ...
        public static void Generate()
        {
            ...
            using (var writer = new StreamWriter(filePath)) {
                ...
                writer.WriteLine("// UnityConstantsGenerator.cs modified by Janus Kjempff"); 
                writer.WriteLine("//    - documentation parts removed for readability.");
                writer.WriteLine();
...
//                    writer.WriteLine("        /// </summary>");
                ...
                //// Write out the names .. well not, couldn't get it working
                //writer.WriteLine("    public static class Names");
                //writer.WriteLine("    {");
                ////Object[] objs = AssetDatabase.LoadAllAssetsAtPath("Assets");
                ////foreach (var obj in objs) {
                ////    writer.WriteLine("        public const string {0} = \"{1}\";", MakeSafeForCode(obj.name), obj.name);
                ////}
                //string[] strs = AssetDatabase.FindAssets("* t:GameObject"); //this only finds prefabs ... hmm how to find all gameobjects?
                //foreach (var strr in strs) {
                //    string str = AssetDatabase.GUIDToAssetPath(strr);
                //    writer.WriteLine("        public const string {0} = \"{1}\";", MakeSafeForCode(str), str);
                //}
                //writer.WriteLine("    }");
                //writer.WriteLine();
